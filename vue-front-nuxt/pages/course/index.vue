...
export default {
  ...
  methods: {
    initCourseFirst() {
      courseApi.getCourseLsit(1, 8, this.searchObj)
        .then(response => {
    ...
    //  分页切换
    gotoPage(page) {
      courseApi.getCourseLsit(page, 8, this.searchObj)
        .then(response => {
          this.data = response.data.data
        })
    },
    ...
    searchOne(subjectParentId, index) {
      ...
      this.subSubjectList = []
      // 把一级分类点击id值，赋值给 searchobj
      this.searchObj.subjectParentId = subjectParentId
      // 点击某个一级分类进行条件查询
      this.gotoPage(1)
      // 拿着点击的一级分类的id和一级所有分类比较
