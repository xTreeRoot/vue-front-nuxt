          ...
          <tr class="good">
            <td class="name First">
              ...
              <div class="goodInfo">
                ...
                <a
                  :href="'https://localhost:3000/course/'+
                  order.courseId"
                  target="_blank">{{ order.courseTitle }}</a>
              </div>
            </td>
            <td class="price">
...
f20">￥<strong
        ...
        <button id="submitPay" class="fr redb" type="button" @click="toPay()">去支付
        </button>
        <div class="clear"/>
      </div>
    </form>
...
export default {
  asyncData({ parms, error }) {
    orderApi.getOrdersInfo(parms.oid)
      .then(response => {
        return {
